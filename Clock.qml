...
Window{
    ...
    GFile{//文件操作
        ...
        function read_(){//判断是否是第一次启动
            var a=0
            file.source="./is_clock.txt"
            if(file.is(file.source))
            {
                file.read2("./value_clock.txt")//读取保存的状态
            }
            if(a==0)//是第一次启动
            {
                file.source="./is_clock.txt"
        ...
        Item {
            ...
            Rectangle{
                ...
                Cbutton{
                    ...
                    onClicked: {
                        enabled=false
                        file.source="./file/saves_clock/"+save_text.text+".txt"
                        file.save2(file.source)
                        var Csaves=Qt.createComponent("./CSaveItemC.qml"),im
