...
Image{//游戏界面
    ...
    function generate_food()
    {
        ...
        if(type==0 ||type==3)
        {
            ...
            {
                im_type=1
                image =im= Qt.createQmlObject("import QtQuick 2.14; Image { z:2147483647; source: './images/food_speed.png'; width: 20; height: 20}", item);
            }
            else if(lxj>0.85)
            {
                im_type=10
                image =im= Qt.createQmlObject("import QtQuick 2.14; Image { z:2147483647; source: './images/food_speed_big.png'; width: 20; height: 20}", item);
            }
            else if(lxj>0.75)
            {
                im_type=2
                image =im= Qt.createQmlObject("import QtQuick 2.14; Image { z:2147483647; source: './images/food_throuth.png'; width: 20; height: 20}", item);
            }
            else if(lxj>0.7)
            {
                im_type=20
                image =im= Qt.createQmlObject("import QtQuick 2.14; Image { z:2147483647; source: './images/food_throuth_big.png'; width: 20; height: 20}", item);
            }
            else if(lxj>0.65)
            {
                im_type=3
                image =im= Qt.createQmlObject("import QtQuick 2.14; Image { z:2147483647; source: './images/food_high.png'; width: 20; height: 20}", item);

            }
            else if(lxj>0.6)
            {
                im_type=4
                image =im= Qt.createQmlObject("import QtQuick 2.14; Image { z:2147483647; source: './images/food_s&t.png'; width: 20; height: 20}", item);

            }
            else
            {
                im_type=0
                image =im= Qt.createQmlObject("import QtQuick 2.14; Image { z:2147483647; source: './images/food.png'; width: 20; height: 20}", item);
            }
    ...
    Component.onCompleted: {
        ...
        for(i=0;i<800;i+=20)//
        {
           for(j=0;j<600;j+=20)
            {
                var image = Qt.createQmlObject("import QtQuick 2.14; Image { z:-1; source: './images/back.png'; width: 20; height: 20}", item);
                image.x=i
                image.y=j
            }
        }
            ...
            onTriggered:{
                ...
                if(!pausen)
                {
                    ...
                    if(hear.x==x0 && hear.y==y0 && isEat)//吃到食物
                    {
                        ...
                        snack.cent++
                        snack.num++
                        var image=snack.body[snack.num]= Qt.createQmlObject("import QtQuick 2.14; Image { z:2; source: './images/snack_body1.png'; width: 20; height: 20}", item);
                        var aa
                        if(snack.num>=1)
    ...
    Keys.onReleased: (event)=>{
        ...
        {
            win.title="SnackQML//开发者模式 ("+thisnum+")"
            control=true
            press_su.play_()
        }
